...
class Xml3176Xml2Checker
{
    ...
    protected function setConditions()
    {
        ...
        $this->drugGroups = config('xml3176.drug_group_code'); // Mã nhóm thuốc cần kiểm tra
        $this->bloodGroup = config('xml3176.blood_group_code'); // Mã nhóm máu
        $this->excludedDepartments = config('organization.exclude_department'); //Mã khoa không cần kiểm tra
        $this->drugGroupNotCheck = config('xml3176.drug_group_not_check');
        $this->drugCodeNotCheck = config('xml3176.drug_code_not_check'); // Các mã thuốc cụ thể không cần kiểm tra
    }
     ...
     * @return void
     */
    public function checkErrors(Xml3176Xml2 $data): void
    {
    ...
    private function infoChecker(Xml3176Xml2 $data): Collection
    {
        $errors = collect();
        ...
        if (!empty($data->tt_thau)) {
            ...
            if (count($ttThauParts) == 4) {
                ...
                $config = config('xml3176.xml2.tt_thau');
     ...
     * @return Collection
     */
    private function checkOrderTime(Xml3176Xml2 $data): Collection
    {
        $errors = collect();
     ...
     * @return Collection
     */
    private function checkMedicalStaff(Xml3176Xml2 $data): Collection
    {
        $errors = collect();
     ...
     * @return Collection
     */
    private function checkDrugCatalog(Xml3176Xml2 $data): Collection
    {
        $errors = collect();
    ...
    private function checkValidMakhoaReq(Xml3176Xml2 $data): Collection
    {
        $errors = collect();
        ...
        if (in_array($data->Xml3176Xml1->ma_loai_kcb, config('xml3176.treatment_type_inpatient'))) {
            ...
            if ($is_ma_doituong_kcb_trai_tuyen && 
                in_array($data->ma_khoa, config('xml3176.general.ma_khoa_kkb'))) {
    ...
    // Kiểm tra phạm vi
    private function checkValidPhamVi(Xml3176Xml2 $data): Collection
    {
        $errors = collect();
    ...
    private function isPhamViInvalid(Xml3176Xml2 $data): bool
    {
        ...
        $maTheBhyt = $data->Xml3176Xml1->ma_the_bhyt;
