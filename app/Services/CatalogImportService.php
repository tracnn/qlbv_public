...
class CatalogImportService
{
    ...
    private function importMedicalStaff($data)
    {
        ...
        foreach ($data as $row) {
            if (empty($row[2]) || empty($row[3]) || empty($row[4]) || empty($row[5]) || empty($row[7]) || empty($row[9]) || empty($row[10]) || empty($row[11]) || empty($row[16]) || empty($row[22])) {
