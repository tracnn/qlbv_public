...
module.exports = function part7() {
  return [
    ...
    p('Khi bấm từ Dashboard sang, ô chọn khoảng ngày trên màn Danh sách hồ sơ sẽ hiện một khoảng rất rộng thay vì Hôm nay như mặc định. Đó là chủ ý chứ không phải lỗi: ô trên Dashboard nói về toàn bộ thời gian, nên nếu giữ nguyên khoảng ngày mặc định thì bấm vào một ô đã tiếp nhận từ tháng trước sẽ ra danh sách rỗng — đúng cái ô vừa báo là gửi thành công. Muốn thu hẹp lại thì chọn khoảng ngày như bình thường rồi bấm Tải dữ liệu.'),
    p('Cột của cơ sở không còn trong danh sách cơ sở hiện hành được đánh dấu riêng trên đầu cột. Dữ liệu cũ vẫn xem được, nhưng người đọc cần biết mình đang nhìn một cơ sở không còn hoạt động. Khi phần mềm không đọc được danh sách cơ sở từ hệ thống HIS thì không cột nào bị đánh dấu, và màn hình báo rõ là đang thiếu danh sách — vì lúc đó phần mềm không biết cơ sở nào còn cơ sở nào không, mà đánh dấu nhầm cả bảng còn tệ hơn không đánh dấu gì.'),
