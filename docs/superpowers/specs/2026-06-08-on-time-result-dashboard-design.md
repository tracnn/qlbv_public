...
- **TG hẹn** = `his_service.estimate_duration`
...
Biểu đồ dùng cùng thư viện chart đang dùng trong các dashboard hiện có (ApexCharts/Chart.js — theo dashboard sẵn có trong dự án).
...
`buildDetailSqlAndBindingsOnTimeResult` trả SQL liệt kê từng dòng sere_serv (join service_req, service, treatment, execute_room) với cột:

| Cột | Nguồn |
|---|---|
| Mã/Tên BN | `tdl_treatment_code`, `tdl_patient_name` (hoặc từ treatment) |
| Khoa/phòng thực hiện | `his_execute_room.execute_room_name` |
| Loại dịch vụ | tên theo `tdl_service_type_id` |
| Tên dịch vụ | `tdl_service_name` / `his_service.service_name` |
...
Click ở bảng tổng hợp → JS set các filter này → reload DataTables.
...
Thêm vào nhóm `khth` trong `routes/web.php`:

```php
Route::get('on-time-result-index', 'KHTH\OnTimeResultController@index')->name('khth.on-time-result-index');
Route::get('on-time-result/summary', 'KHTH\OnTimeResultController@getSummary')->name('khth.on-time-result-summary');
Route::get('on-time-result/fetch', 'KHTH\OnTimeResultController@fetch')->name('khth.on-time-result-fetch');
Route::get('on-time-result/export', 'KHTH\OnTimeResultController@export')->name('khth.on-time-result-export');
Route::get('on-time-result/rooms', 'KHTH\OnTimeResultController@rooms')->name('khth.on-time-result-rooms'); // dropdown phòng (nếu cần)
```
...
Thêm 1 mục trong nhóm KHTH ở `config/adminlte.php`:
...
- Hiệu năng: filter bắt buộc theo `intruction_time` (đã đánh chỉ mục theo pattern các report khác); CTE gom 1 lần rồi tổng hợp nhiều chiều.
