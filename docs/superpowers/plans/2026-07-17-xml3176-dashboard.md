...
public function getAging(): array   // D11: KHÔNG nhận bộ lọc kỳ — xem ghi chú bên dưới
public function getByDepartment(string $dateType, string $dateFrom, string $dateTo): array
```

### ⚠️ Cập nhật D11 (phát hiện khi triển khai Task 5)

`getAging()` **không nhận bộ lọc kỳ** và luôn tính theo `ngay_ra` so với hôm nay.

Lý do: khối này vốn lọc **hai** điều kiện ngày cùng lúc (kỳ theo `date_type` + `ngay_ra` theo nhóm tuổi), trong khi màn hình danh sách chỉ nhận **một** `date_type` + một khoảng ngày → drill-down không tái hiện được → số trên cột lệch với danh sách, vi phạm nguyên tắc cốt lõi. Bỏ ràng buộc kỳ thì chỉ còn một điều kiện trên `ngay_ra` → khớp tuyệt đối. Cũng đúng nghiệp vụ: tồn đọng là câu hỏi "hiện tại".

Hệ quả cho các task sau:
- **Task 6:** endpoint `aging` **không validate** `date_type`/`date_from`/`date_to` (không dùng tới), và gọi `$this->service->getAging()` không tham số.
- **Task 7:** `loadAging()` **không gửi** tham số ngày; drill-down truyền đúng khoảng ngày của nhóm tuổi, **không** kèm bộ lọc kỳ; biểu đồ **phải ghi nhãn** "không phụ thuộc khoảng ngày đã chọn".
...
```php
    ...
    public function getAging(): array
    {
        ...
        foreach ($buckets as $bucket) {
            $query = $this->scopeNotSubmitted(Xml3176Xml1::query())
...
Không cần `whereNotNull('ngay_ra')->where('ngay_ra','<>','')`: `whereBetween` đã loại cả `NULL` (logic ba trị → UNKNOWN) lẫn chuỗi rỗng (`'' < '000000000000'` ở mọi bucket).
...
class Xml3176DashboardControllerTest extends TestCase
{
    ...
    {
        // D11: getAging() không nhận tham số — withNoArgs() khoá hợp đồng đó.
        // Nếu ai đó lỡ truyền lại bộ lọc kỳ vào, test này sẽ đỏ.
        $mock = Mockery::mock(Xml3176DashboardService::class);
        $mock->shouldReceive('getAging')->once()->withNoArgs()->andReturn([
...
(function (win, $) {
    ...
    // D11: KHÔNG gửi tham số ngày — endpoint aging không nhận và không validate chúng.
    // Cũng KHÔNG dùng openList() cho drill-down của khối này, vì openList() luôn merge
    // getParams() vào URL; khối tồn đọng phải tự dựng querystring chỉ với khoảng ngày của bucket.
    function loadAging() {
