...
class Tt12SubmitService
{
    ...
    private function motLanGui($url, $loaiHs, $base64, $maCskcb, BHYTLoginService $login)
    {
        ...
        $body = array(
            ...
            $truong['ma_tinh']     => (string) config('organization.tt12.ma_tinh'),
...
- `index.blade.php` ← từ `ctdt/index.blade.php`: đổi `url` DataTable thành `route('bhyt.tt12.fetch-data')`, đổi tập cột theo `dongDanhSach()` ở Step 7, đổi hai ô lọc thành **Mẫu** (`$danhSachMau`) và **Trạng thái** (`$cacTrangThai`), đổi route nút hàng loạt thành `bhyt.tt12.ky-va-gui-nhieu`.
