...
class CreateXml3176Xml1sTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('xml3176_xml1s', function (Blueprint $table) {
            $table->increments('id');
            $table->string('ma_lk');
            $table->integer('stt');
            ...
            // Adding index
            $table->unique(['ma_lk', 'stt'], 'xml3176_xml1s_ma_lk_stt_unique');
            $table->index('ma_lk');
            $table->index('ma_bn');
    ...
    {
        Schema::dropIfExists('xml3176_xml1s');
    }
}
