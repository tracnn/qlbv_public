...
<div class="panel panel-default">
    <div class="panel-body">
        <form action="{{ route('system.do-upload-xml') }}" class="dropzone" id="xmlUploadForm">
            {{ csrf_field() }}
        </form>
    </div>
</div>
...
<script>
    Dropzone.options.xmlUploadForm = {
        ...
        init: function() {
            ...
            function updateFileRow(row, file, status) {
                if (status === 'success') {
                    row.cells[2].innerHTML = '<i class="fa fa-check-circle" style="color: green;"></i>';
                } else if (status === 'error') {
                    row.cells[2].innerHTML = '<i class="fa fa-times-circle" style="color: red;"></i>';
