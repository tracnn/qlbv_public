...
:: Tạo task mới: Chạy mỗi ngày vào lúc 00:00 (12h đêm)
:: /sc daily: Chạy hàng ngày
:: /st 00:00: Thời điểm bắt đầu chạy
:: /ru "SYSTEM": Chạy với quyền hệ thống (không hiển thị cửa sổ CMD gây phiền người dùng)
:: /f: Ép buộc tạo
schtasks /create /tn "%SCRIPT_NAME%" /tr "powershell.exe -ExecutionPolicy Bypass -WindowStyle Hidden -File \"%SCRIPT_PATH%\"" /sc daily /st 00:00 /ru "SYSTEM" /f

if %ERRORLEVEL% EQU 0 (
    echo [OK] Da dang ky thanh cong. He thong se tu dong kiem tra update luc 00:00 moi ngay.
